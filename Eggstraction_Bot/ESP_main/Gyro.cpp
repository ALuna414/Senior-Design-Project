...
float Gyro::relativeAngle(float heading)
{
  return relativeAngle(angle(), heading);
}

float Gyro::relativeAngle(float reference, float heading)
{
  float error = heading - reference; 
  if(error < -180)
    error += 360;
  else if(error > 180)
    error -= 360;
